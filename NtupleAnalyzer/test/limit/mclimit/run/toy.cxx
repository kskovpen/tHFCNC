...
//	h_data[i] = (TH1F*)f[i]->Get("data");
	...
	int nbins = h_signal[i]->GetXaxis()->GetNbins();
	
	if( nbins > 0 && nbins != nbins0 )
	  {	     
	     nbins0 = h_signal[i]->GetXaxis()->GetNbins();
	     std::cout << nbins << " " << nbins0 << std::endl;
	  }	
     }         
...
/*	sample_1_sysname[nmax*i+2] = Form("mu_scale_sample_1_%d",i);
	sample_1_sysname[nmax*i+3] = Form("el_smear_sample_1_%d",i);
	sample_1_sysname[nmax*i+4] = Form("el_PSStat_sample_1_%d",i);
	sample_1_sysname[nmax*i+5] = Form("el_R12Stat_sample_1_%d",i);
*/	
	sample_2_sysname[nmax*i] = Form("JES_sample_2_%d",i);
	sample_2_sysname[nmax*i+1] = Form("JER_sample_2_%d",i);
/*	sample_2_sysname[nmax*i+2] = Form("mu_scale_sample_2_%d",i);
	sample_2_sysname[nmax*i+3] = Form("el_smear_sample_2_%d",i);
	sample_2_sysname[nmax*i+4] = Form("el_PSStat_sample_2_%d",i);
	sample_2_sysname[nmax*i+5] = Form("el_R12Stat_sample_2_%d",i);
*/	
	sample_3_sysname[nmax*i] = Form("JES_sample_3_%d",i);
	sample_3_sysname[nmax*i+1] = Form("JER_sample_3_%d",i);
/*	sample_3_sysname[nmax*i+2] = Form("mu_scale_sample_3_%d",i);
	sample_3_sysname[nmax*i+3] = Form("el_smear_sample_3_%d",i);
	sample_3_sysname[nmax*i+4] = Form("el_RSStat_sample_3_%d",i);
	sample_3_sysname[nmax*i+5] = Form("el_R12Stat_sample_3_%d",i);
*/
...
/*	signal_sysname[nmax*i+2] = Form("mu_scale_signal_%d",i);
	signal_sysname[nmax*i+3] = Form("el_smear_signal_%d",i);
	signal_sysname[nmax*i+4] = Form("el_RSStat_signal_%d",i);
	signal_sysname[nmax*i+5] = Form("el_R12Stat_signal_%d",i);
*/	
	sample_1_sysname_tf[0] = "jes";
	sample_1_sysname_tf[1] = "jer";
/*	sample_1_sysname_tf[2] = "mu_scale";
	sample_1_sysname_tf[3] = "el_smear";
	sample_1_sysname_tf[4] = "el_PSStat";
	sample_1_sysname_tf[5] = "el_R12Stat";
*/	
	sample_2_sysname_tf[0] = "jes";
	sample_2_sysname_tf[1] = "jer";
/*	sample_2_sysname_tf[2] = "mu_scale";
	sample_2_sysname_tf[3] = "el_smear";
	sample_2_sysname_tf[4] = "el_PSStat";
	sample_2_sysname_tf[5] = "el_R12Stat";
*/	
	sample_3_sysname_tf[0] = "jes";
	sample_3_sysname_tf[1] = "jer";
/*	sample_3_sysname_tf[2] = "mu_scale";
	sample_3_sysname_tf[3] = "el_smear";
	sample_3_sysname_tf[4] = "el_PSStat";
	sample_3_sysname_tf[5] = "el_R12Stat";
*/	
...
/*	signal_sysname_tf[2] = "mu_scale";
	signal_sysname_tf[3] = "el_smear";
	signal_sysname_tf[4] = "el_PSStat";
	signal_sysname_tf[5] = "el_R12Stat";*/
     }  
   ...
   for(int j=0;j<nf;j++)
	...
	if( j == 1 ) chan = chan2;
	if( j == 2 ) chan = chan3;
	if( j == 3 ) chan = chan4;//new??
	if( j == 4 ) chan = chan5;//new??
	if( j == 5 ) chan = chan6;//new
...
//	PRIOR prior; // corr or flat
	...
	if( bayf == 1 )
	  {	     
	     mymclimit->bayes_heinrich(bh,&sflimit,&unc);
	     mymclimit->bayes_heinrich_withexpect(bh,&sflimit,&unc,npe,&sm2,&sm1,&smed,&sp1,&sp2);
	     
	     std::cout << "Expected Limit at: " << bh <<" % C.L "<< smed*sfsig << " Full band: -2SD -1SD MED +1SD +2SD " << sm2*sfsig << " " << sm1*sfsig << " " << smed*sfsig << " " << sp1*sfsig << " " << sp2*sfsig << std::endl;
	     std::cout << "Observed Limit at: " << bh <<" % C.L "<< sflimit*sfsig << std::endl;
	  }	
...
*/
   ...
   TH1* myH0 = new TH1F("h0_hypo","tPDF H_{0}",50,-200.,200);
   TH1* myH1 = new TH1F("h1_hypo","tPDF H_{1}",50,-200.,200); 

   TH1* mcb_hist = new TH1F("mcb_hist","mcb_hist",100,-50.,50);
   TH1* mcs_hist = new TH1F("mcs_hist","mcs_hist",100,-50.,50);
   TH1* data_hist = new TH1F("data_hist","data_hist",100,-50.,50);         
   
   mymclimit->tshists(myH1,myH0) ;
   
   TCanvas *tPDF = new TCanvas("tPDF","tPDF");
   myH0->SetLineColor(2);
   myH1->Draw("");
   myH0->Draw("same");
   ...
   TH1 *h_sample_1_sys_p_extrap[50][sample_1_nsys][nsn];
   TH1 *h_sample_1_sys_n_extrap[50][sample_1_nsys][nsn];

   TH1 *h_sample_2_sys_p_extrap[50][sample_2_nsys][nsn];
   TH1 *h_sample_2_sys_n_extrap[50][sample_2_nsys][nsn];
   ...
   for(int j=0;j<nf;j++)
	...
	for(int i=0;i<sample_1_nsys;i++)
	  {   
	     h_sample_1_sys_p[j][i]->Write();
	     h_sample_1_sys_n[j][i]->Write();
	     
	     for(int k=0;k<nsn;k++)
	       {		  
		  h_sample_1_sys_p_extrap[j][i][k] = (TH1*)h_sample_1_sys_p[j][i]->
		    Clone("sample_1_sys_"+TString(sample_1_sysname_tf[i])+"_p_extrap_s_"+TString(Form("%d",int(ns[k]*10.))));
		  csm_interpolate_histogram(h_sample_1[j],0.,h_sample_1_sys_p[j][i],1.,h_sample_1_sys_p_extrap[j][i][k],
					    ns[k],CSM_INTERP_VERTICAL_EXTRAP);

		  h_sample_1_sys_n_extrap[j][i][k] = (TH1*)h_sample_1_sys_n[j][i]->
		    Clone("sample_1_sys_"+TString(sample_1_sysname_tf[i])+"_n_extrap_s_"+TString(Form("%d",int(ns[k]*10.))));
		  csm_interpolate_histogram(h_sample_1[j],0.,h_sample_1_sys_n[j][i],1.,h_sample_1_sys_n_extrap[j][i][k],
					    ns[k],CSM_INTERP_VERTICAL_EXTRAP);
	       }	     
	  }	
	...
	for(int i=0;i<sample_2_nsys;i++)
	  {   
	     h_sample_2_sys_p[j][i]->Write();
	     h_sample_2_sys_n[j][i]->Write();
	     
	     for(int k=0;k<nsn;k++)
	       {		  
		  h_sample_2_sys_p_extrap[j][i][k] = (TH1*)h_sample_2_sys_p[j][i]->
		    Clone("sample_2_sys_"+TString(sample_2_sysname_tf[i])+"_p_extrap_s_"+TString(Form("%d",int(ns[k]*10.))));
		  csm_interpolate_histogram(h_sample_2[j],0.,h_sample_2_sys_p[j][i],1.,h_sample_2_sys_p_extrap[j][i][k],
					    ns[k],CSM_INTERP_VERTICAL_EXTRAP);

		  h_sample_2_sys_n_extrap[j][i][k] = (TH1*)h_sample_2_sys_n[j][i]->
		    Clone("sample_2_sys_"+TString(sample_2_sysname_tf[i])+"_n_extrap_s_"+TString(Form("%d",int(ns[k]*10.))));
		  csm_interpolate_histogram(h_sample_2[j],0.,h_sample_2_sys_n[j][i],1.,h_sample_2_sys_n_extrap[j][i][k],
					    ns[k],CSM_INTERP_VERTICAL_EXTRAP);
	       }	     
	  }	
     }   
